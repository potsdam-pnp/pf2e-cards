...
\NewDocumentCommand{\DamageRoll}{omomm}{
  \tikz[baseline=(content.base)]{
    \node (content) {
      \strut\smash{
        \IfValueTF{#1}{\mbox{#1}}{}
        \IfValueTF{#3}
          {%
            ...
            \raisebox{-0.4em}{\DamageType{#3}}%
          }
          {\ifnum\pdfstrcmp{#2}{}=0 \else\DamageType{#2}\fi}
        \(
          \mbox{#4}
          \foreach \part in {#5} {
            + \FormulaVariable{\part}{\phantom{10}}
          }
        \)
      }
    };
...
\NewDocumentCommand{\HealingRoll}{omm}{
  \tikz[baseline=(content.base)]{
    \node (content) {
      \strut\smash{
        \(
          \IfValueTF{#1}{\mbox{#1}}{}
          \mbox{#2}
          \foreach[count=\index] \part in {#3} {
            \pgfmathparse{\index > 1 || \pdfstrcmp{#2}{} != 0}
